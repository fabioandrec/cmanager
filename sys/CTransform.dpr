    ...
    if GetSwitch('-h') then begin
      ...
      if xXmlFilename = '' then begin
        ...
        if not FileExists(xXmlFilename) then begin
          ...
          xXmlDoc := GetDocumentFromFile(xXmlFilename, Nil);
          if xXmlDoc <> Nil then begin
            if xXmlDoc.parseError.errorCode = 0 then begin
              xXslDoc := GetDocumentFromFile(xXslFilename, Nil);
              if xXslDoc <> Nil then begin
                ...
                xText := 'Nie mo¿na odczytac pliku ' + xXslFilename + ', ' +  SysErrorMessage(GetLastError);
              end;
            end else begin
