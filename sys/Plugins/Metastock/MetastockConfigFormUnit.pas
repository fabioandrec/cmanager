...
begin
  if ListView.Selected <> Nil then begin
    if MessageBox(0, 'Czy usunπÊ wybranego dostawcÍ?', 'Pytanie', MB_YESNO + MB_ICONQUESTION) = ID_YES then begin
...
begin
  if MessageBox(0, 'Czy przywrÛciÊ domyúlne ustawienia dostawcÛw?', 'Pytanie', MB_YESNO + MB_ICONQUESTION) = ID_YES then begin
    xConfigurationDecoded := GetStringFromResources('DEFAULTXML', RT_RCDATA);
